...
allowed_down_effects = ['down-regulates', 'down-regulates activity', 'down-regulates quantity',
                      'down-regulates quantity by repression', 'down-regulates quantity by destabilization']
allowed_up_effects = ['up-regulates', 'up-regulates activity', 'up-regulates quantity',
                        'up-regulates quantity by expression', 'up-regulates quantity by stabilization']

# mapping names to exact strings in SIGNOR database, any one of these matching will trigger a match
name_map = { "CycD": ["CyclinD/CDK4", "CDK4", 'CDK6', 'CCND1', 'CDK6/CCND1'],
    ...
    for c1_name in name_map[c1]:
        for c2_name in name_map[c2]:
            if ((c1_name, c2_name) in signor_data_map):
                ...
                if effect==0: effect_0_valid = False
                
                effect_in_db, pmid = signor_data_map[(c1_name, c2_name)]
                # for effect 1 or -1, we can immediately return True when found
                if (effect_in_db in allowed_up_effects and effect==1) \
                    or (effect_in_db in allowed_down_effects and effect==-1):
                    item = (c1_name, c2_name, effect, pmid,)
...
def map_cols_to_dbContext(value):
    pert = value.strip()
    if pert in perturbs_result:
        db_context_string = ' '.join(map(lambda x:'[%s -> %s -> %s -> %s]' % x, perturbs_result[pert].get('db_context', [])))
...
if __name__ == "__main__":
    ...
    for row in model_perturb_data[cols].to_dict('records'):
        ...
        if PRINT_SCORE: score = row['Graph Score']
        db_context = []
        # if perturb in results and results[perturb]: continue # if perturb seen before, dont process again
        
        if '->' not in perturb: print('Skipping perturb: ' + perturb); continue
    ...
    print("%d out of %d perturbs checked are in DB" % (len(list(filter(lambda x:x['valid'], perturbs_result.values()))), len(perturbs_result.values())))
